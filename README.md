...
Small regional datasets pose a dual statistical challenge: correlated predictors inflate estimation variance, while highly flexible machine learning models can become unstable due to limited information per adaptive degree of freedom. This project investigates this phenomenon—termed **predictive volatility**—using both simulated environments and an empirical application predicting provincial poverty rates across 34 Indonesian provinces[cite: 4]. 

The codebase evaluates a wide range of models, including:
* **Frequentist Shrinkage Models:** OLS, Ridge, LASSO, Elastic Net[cite: 4].
* **Bayesian Parametric Models:** Gaussian LM, Bayesian Ridge, Bayesian LASSO, Horseshoe priors, and Beta regression[cite: 4].
* **Spatial Diagnostics:** K-nearest neighbors spatial graph (BYM2/ICAR)[cite: 4].
* **Machine Learning Ensembles:** Random Forest, XGBoost, BART, and Gaussian Processes[cite: 4].

## Repository Structure

* `data/`: Contains the cleaned, aggregated 2021 province-level dataset sourced from Badan Pusat Statistik (BPS) Indonesia (n=34)[cite: 4].
* `scripts/`: Contains the master replication R scripts for data preparation, parametric and non-parametric cross-validation loops, prior sensitivity analysis, and the multi-scenario asymptotic simulation grid[cite: 4].
* `figures/`: Automatically generated directory where the scripts output the tables, predictive distributions, and SHAP summary plots[cite: 4].

## System Requirements and Dependencies

All analyses, simulations, and visualizations were programmed and executed in the **R statistical computing environment**[cite: 4]. Bayesian MCMC sampling chains were compiled and executed via Stan[cite: 4]. 

To run the master replication script, ensure you have the following R packages installed[cite: 4]:
