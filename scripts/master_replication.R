...
#   "ggcorrplot", "bayesplot", "posterior", "BoomSpikeSlab"
# ))
...
# Spatial graph: Dynamic k-nearest neighbors to ensure a fully connected network
coords_matrix <- as.matrix(real_coords)

k_val <- 2
is_connected <- FALSE

# Loop to find the minimum k that eliminates isolated islands
while (!is_connected && k_val < nrow(coords_matrix)) {
  knn_nb <- knn2nb(knearneigh(coords_matrix, k = k_val))
  sym_nb <- make.sym.nb(knn_nb)
  
  # n.comp.nb checks the number of connected subgraphs; we want exactly 1
  if (n.comp.nb(sym_nb)$nc == 1) {
    is_connected <- TRUE
  } else {
    k_val <- k_val + 1
  }
...
cat("Graph successfully connected using k =", k_val, "\n")
...
m11_icar <- brm(
  formula = Y ~ X1 + X2 + X3 + X4 + X5 + X6 + X7 + X8 + X9 +
    car(A, gr = spatial_id, type = "icar"), # Simplified from BYM2 to ICAR
  data = df,
  data2 = list(A = A),
  family = gaussian(),
  prior = c(
    ...
    set_prior("normal(0,5)", class = "b"),
    set_prior("inv_gamma(3,2)", class = "sigma"),
    set_prior("exponential(1)", class = "sdcar") # Tightened spatial prior
  ),
  chains = 4,
  iter = 4000,   
  warmup = 2000, 
...
# ==============================================================================
# 3.b SPIKE-AND-SLAB ESTIMATION (M9) FOR EXACT PIP GENERATION
# ==============================================================================
cat("--- Step 3.b: Compiling Spike-and-Slab Model (M9) ---\n")
...
spike_summary <- summary(m9_spike)

m9_results <- data.frame(
  Predictor = rownames(spike_summary$coefficients),
  PIP = round(spike_summary$coefficients[, "inc.prob"], 3),
  Mean_Conditional = round(spike_summary$coefficients[, "mean"], 3)
)

m9_results <- m9_results[m9_results$Predictor != "(Intercept)", ]
m9_results$Predictor <- c("X1", "X2", "X3", "X4", "X5", "X6", "X7", "X8", "X9")

write.csv(m9_results, file.path(mainDir, "1. Findings", "Table3_M9_SpikeSlab_PIP.csv"), row.names = FALSE)

# 3.c MCMC CONVERGENCE DIAGNOSTICS
# 3.c MCMC CONVERGENCE DIAGNOSTICS
# ==============================================================================
cat("--- Step 3.c: Extracting MCMC convergence diagnostics ---\n")

extract_convergence_bounds <- function(brms_model, model_name, model_id) {
  # Let the function compute the default summary to guarantee standard column names
  ss <- suppressWarnings(posterior::summarise_draws(brms_model))
  
  # Extract only finite values 
  rhats <- ss$rhat[is.finite(ss$rhat)]
  neffs <- ss$ess_bulk[is.finite(ss$ess_bulk)]
  
  # Strict fallback logic to prevent Inf / -Inf if lists ever return empty
  min_rhat <- if (length(rhats) > 0) min(rhats) else NA
  max_rhat <- if (length(rhats) > 0) max(rhats) else NA
  min_ess  <- if (length(neffs) > 0) min(neffs) else NA
  max_ess  <- if (length(neffs) > 0) max(neffs) else NA
  
  data.frame(
    ID = model_id,
    Model = model_name,
    Min_Rhat = round(min_rhat, 3),
    Max_Rhat = round(max_rhat, 3),
    Min_ESS  = round(min_ess, 0),
    Max_ESS  = round(max_ess, 0)
  )
}
...
preds_spike <- numeric(n_emp) # M9 Integration
...
for (i in seq_len(n_emp)) {
  ...
  # Spike-and-slab (M9 Integration)
  spike_cv <- lm.spike(y ~ ., data = data.frame(y = y_tr, x_tr), niter = 2000, ping = 0)
  preds_spike[i] <- mean(predict(spike_cv, newdata = data.frame(x_te)))
...
tab_empirical <- data.frame(
  ID = c("M1", "M2", "M3", "M4", "M5", "M6", "M7", "M8", "M9", "M10", "M11", "M12", "M13", "M14", "M15"),
  Model = c(
    "OLS", "Ridge", "LASSO", "Elastic net",
    "Bayes LM (Gaussian)", "Bayesian ridge", "Bayesian LASSO", "Horseshoe", "Spike-and-slab",
